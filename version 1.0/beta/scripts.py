...
async def check_server_status(screen_name, expected_nicknames=None):
    ...
    try:
        ...
        if stdout:
            ...
            if stdout:
                ...
                active_players = set()
                connected_nicknames = set()
                ct_score = 0
                t_score = 0
                game_ended = False
                nicknames_correct = True
                        ...
                        if 'active' in line.lower():
                            ...
                            if len(parts) >= 5:
                                try:
                                    ...
                                    connected_nicknames.add(player_name)
                                except Exception as e:
                ...
                # Проверяем ники только если переданы ожидаемые ники
                if expected_nicknames and len(active_players) == len(expected_nicknames):
                    nicknames_correct = all(nick in connected_nicknames for nick in expected_nicknames)
                    if not nicknames_correct:
                        # Отправляем сообщение в чат сервера
                        await ssh_manager.execute_command(
                            f'screen -S {screen_name} -X stuff \'say "Неизвестные ники, пожалуйста замените их на те что указаны в профиле"\\n\''
                        )
                ...
                return {
                    ...
                    'nicknames_correct': nicknames_correct
                }
...
async def monitor_server(screen_name, player_ids, expected_nicknames):
    ...
    match_logger.info(f"Ожидаемые ники: {expected_nicknames}")

    start_time = datetime.now().timestamp()
    empty_server_checks = 0
    warned_times = set()
    match_started = False
    initial_connection_time = 500
    check_interval = 10
    last_nickname_warning = 0
    nickname_check_start = datetime.now().timestamp()
    server_join_start = datetime.now().timestamp()
    nickname_warning_count = 0
    ...
    try:
        while True:
            ...
            if not match_started and (current_time - server_join_start) >= config.SERVER_JOIN_TIME:
                match_logger.info("Время на подключение истекло, проверяем неподключившихся игроков")
                await check_and_ban_inactive_players(screen_name, player_ids)
                break
            ...
            status = await check_server_status(screen_name, expected_nicknames)
            if not status:
            ...
            # Проверка ников и запуск матча
            if not match_started and status['active_players'] >= len(player_ids):
                if not status['nicknames_correct']:
                    # Проверяем время на исправление ников
                    if (current_time - nickname_check_start) >= config.NICKNAME_CHECK_TIME:
                        match_logger.info("Время на исправление ников истекло, применяем баны")
                        # Получаем список игроков с неправильными никами
                        wrong_nickname_players = await get_wrong_nickname_players(
                            screen_name, expected_nicknames
                        )
                        for player_id in wrong_nickname_players:
                            await apply_search_ban(
                                str(player_id),
                                "Автоматический бан: Неправильный ник в игре"
                            )
                        await stop_server(screen_name)
                        match_logger.info("Сервер остановлен из-за неправильных ников")
                        break

                    # Отправляем предупреждение каждые 10 секунд
                    if current_time - last_nickname_warning >= 10:
                        await ssh_manager.execute_command(
                            f'screen -S {screen_name} -X stuff \'say "Неизвестные ники, '
                            f'пожалуйста замените их на те что указаны в профиле. '
                            f'Осталось {int((config.NICKNAME_CHECK_TIME - (current_time - nickname_check_start))/60)} минут"\\n\''
                        )
                        last_nickname_warning = current_time
                        match_logger.info("Отправлено предупреждение о неправильных никах")
                    continue

                # Если все ники правильные, запускаем матч
                match_logger.info("Все игроки подключились с правильными никами, запускаем матч")
                match_started = await start_match(screen_name)
                if match_started:
                    await notify_match_start(player_ids)
                    initial_connection_time = 0
                    match_logger.info("Матч успешно запущен")
...
async def get_wrong_nickname_players(screen_name, expected_nicknames):
    """Получает список игроков с неправильными никами"""
    try:
        stdout, stderr = await ssh_manager.execute_command(f"screen -S {screen_name} -X stuff 'status\\n'")
        await asyncio.sleep(2)
        stdout, stderr = await ssh_manager.execute_command(f"screen -S {screen_name} -X hardcopy /tmp/status_{screen_name}.txt")
        await asyncio.sleep(1)
        stdout, stderr = await ssh_manager.execute_command(f"cat /tmp/status_{screen_name}.txt")
        ...
        for line in stdout.split('\n'):
            if '#' in line and 'STEAM' in line:
                parts = line.split()
                if len(parts) >= 4:
                    player_name = parts[2].strip('"')
                    steam_id = parts[-1]
                    if player_name not in expected_nicknames:
                        # Получаем user_id по steam_id из базы данных
                        cursor.execute("SELECT user_id FROM users WHERE steam_id = ?", (steam_id,))
                        result = cursor.fetchone()
                        if result:
                            wrong_players.append(result[0])

        return wrong_players

    except Exception as e:
        logger.error(f"Ошибка при получении списка игроков с неправильными никами: {e}")
        return []
...
async def finish_search(search_id):
    ...
    try:
        ...
        try:
            # Получаем ники игроков из базы данных
            cursor.execute(
                "SELECT user_id, user_name FROM users WHERE user_id IN ({})".format(
                    ','.join('?' * len(player_ids))
                ),
                player_ids
            )
            player_names = {str(row[0]): row[1] for row in cursor.fetchall()}
            
            # Проверяем, что получили ники для всех игроков
            if len(player_names) != len(player_ids):
                raise Exception("Не удалось получить ники для всех игроков")
                
            logger.info(f"Получены ники игроков: {player_names}")
            
        except Exception as e:
            logger.error(f"Ошибка при получении ников игроков: {e}")
            for user_id, msg in messages_copy.items():
                try:
                    await bot.send_message(
                        msg.chat.id,
                        "⚠️ Ошибка получения данных игроков. Попробуйте позже."
                    )
                except Exception as msg_error:
                    logger.error(f"Ошибка отправки сообщения об ошибке игроку {user_id}: {msg_error}")
            return
        ...
        # Создаем список всех ожидаемых ников
        expected_nicknames = list(player_names.values())
        ...
        server_message = (
            ...
            f"⚠️ Важно: Используйте только указанный ник!"
        )
        ...
        # Запускаем мониторинг сервера с проверкой ников
        asyncio.create_task(monitor_server(screen_name, player_ids, expected_nicknames))
        ...
        logger.info(f"Поиск {search_id} успешно завершен, сервер создан: {screen_name}")
        ...
        for user_id, msg in messages_copy.items():
            try:
                await bot.send_message(
                    msg.chat.id,
                    "⚠️ Произошла ошибка при создании сервера. Попробуйте позже."
                )
            except Exception as msg_error:
                logger.error(f"Ошибка отправки сообщения об ошибке игроку {user_id}: {msg_error}")
    finally:
...
async def apply_search_ban(user_id: str, reason: str):
    """Применяет бан на поиск с увеличением времени"""
    try:
        # Получаем количество предыдущих банов
        cursor.execute("""
            SELECT COUNT(*) FROM bans 
            WHERE user_id = ? AND ban_reason LIKE 'Автоматический бан%'
        """, (user_id,))
        ban_count = cursor.fetchone()[0] + 1  # +1 для текущего бана
        
        # Определяем длительность бана
        ban_hours = config.BAN_TIMES.get(ban_count, config.BAN_TIMES[max(config.BAN_TIMES.keys())])
        ban_end_time = datetime.now() + timedelta(hours=ban_hours)
        
        # Сохраняем бан в базу
        cursor.execute("""
            INSERT INTO bans (user_id, admin_id, ban_reason, ban_date, unban_date)
            VALUES (?, 'system', ?, ?, ?)
        ...
        cursor.execute("""
            ...
            SET ban_status = 1, ban_reason = ?, ban_end_time = ? 
            WHERE user_id = ?
        """, (reason, ban_end_time.strftime('%Y-%m-%d %H:%M:%S'), user_id))
        ...
        await bot.send_message(
            user_id,
            f"⛔️ Вы получили бан на поиск игр на {ban_hours} часов.\n"
            f"Причина: {reason}\n"
            f"Разбан: {ban_end_time.strftime('%Y-%m-%d %H:%M:%S')}"
        )
...
async def check_and_ban_inactive_players(screen_name: str, player_ids: List[str]):
    """Проверяет и банит игроков, которые не подключились к серверу"""
    try:
        status = await check_server_status(screen_name)
        if not status:
            return
            
        connected_players = status['active_players']
        if connected_players < len(player_ids):
            # Банить только тех, кто не подключился
            for player_id in player_ids:
                if str(player_id) not in connected_players:
                    await apply_search_ban(
                        str(player_id),
                        "Автоматический бан: Не подключился к серверу в течение 15 минут"
                    )
                    
    except Exception as e:
        logger.error(f"Ошибка при проверке неактивных игроков: {e}")
